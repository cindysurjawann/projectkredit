...
type Service interface {
	...
	MatchPassword(data LoginRequest) (model.User, int, error)
	UpdatePassword(data LoginRequest) (model.User, int, error)
}
...
func (s *service) MatchPassword(data LoginRequest) (model.User, int, error) {
	userId := data.UserId
	User, err := s.repo.FindUser(userId)
	...
	if err != nil {
		status := http.StatusInternalServerError
		if err.Error() == "user not found" {
			return model.User{}, http.StatusOK, errors.New("user not found")
		}
		return model.User{}, status, err
	}
	...
	err = bcrypt.CompareHashAndPassword([]byte(User.Password), []byte(data.Password))
	if err != nil {
		return model.User{}, http.StatusOK, errors.New("password not match")
	}
	User.Password = ""
	return User, http.StatusOK, nil
}
...
func (s *service) UpdatePassword(data LoginRequest) (model.User, int, error) {
	row, err := s.repo.FindUser(data.UserId)
	if err == nil && row.Name == "" {
		return model.User{}, http.StatusBadRequest, errors.New("user tidak ditemukan")
	}
	...
	res, err := s.repo.UpdatePassword(User)
	if err != nil {
		return model.User{}, http.StatusInternalServerError, err
	}
